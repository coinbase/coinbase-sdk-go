# Go API client for coinbase_client
...
```go
import coinbase_client "github.com/coinbase/coinbase-sdk"
```
...
For using other server than the one defined on index 0 set context value `coinbase_client.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), coinbase_client.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `coinbase_client.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), coinbase_client.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `coinbase_client.ContextOperationServerIndices` and `coinbase_client.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), coinbase_client.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), coinbase_client.ContextOperationServerVariables, map[string]map[string]string{
