...
/*
 * This example code stakes ETH on the Hoodi network via Dedicated ETH Staking and leverages onchain billing https://docs.cdp.coinbase.com/staking/docs/networks/eth-dedicated/overview#billing.
 * Run the code with 'go run examples/ethereum/dedicated-eth-stake-with-onchain-billing/main.go <api_key_file_path> <funding_wallet_address> <reward_splitter_plan_id> <private_key> <rpc_url>'
 */
...
func main() {
	...
	options := []coinbase.StakingOperationOption{
		...
		coinbase.WithFeeRecipientAddress(os.Args[2]),
		coinbase.WithRewardSplitterPlanID(os.Args[3]),
	}
