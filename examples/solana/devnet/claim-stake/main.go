 ...
 * Run the code with 'go run examples/solana/mainnet/claim-stake/main.go <api_key_file_path> <wallet_address> <wallet_private_key>'
 */
...
func main() {
	...
	balance, err := client.GetClaimableBalance(ctx, coinbase.Sol, address)
	if err != nil {
	...
	stakingOperation, err := client.BuildClaimStakeOperation(ctx, amount, coinbase.Sol, address)
	if err != nil {
		log.Fatalf("error building claim stake operation: %v", err)
	}
