...
// reloadStakingOperation reloads a staking operation from the API.
func (c *Client) reloadStakingOperation(ctx context.Context, stakingOperation *StakingOperation) error {
	...
	if httpRes.StatusCode != 200 {
		return fmt.Errorf("failed to fetch staking operation: %s", httpRes.Status)
	}
...
// fetchExternalStakingOperation fetches a staking operation from the API by its ID.
func (c *Client) fetchExternalStakingOperation(ctx context.Context, networkID, addressID, stakingOperationId string) (*StakingOperation, error) {
	so, httpRes, err := c.client.StakeAPI.GetExternalStakingOperation(
		ctx,
		networkID,
		addressID,
